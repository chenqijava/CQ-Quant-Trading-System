...
class MyComponent extends Component {
  ...
  async load(pagination, filters, sorter) {
    ...
    let res = await axios.post(`/api/buyEmailOrder/list/${pagination.pageSize}/${pagination.current}`, { filters:{...filters, type: {$in:[null, 'v1']}}, sorter });
  ...
  render() {
        ...
        render: (t, record, index) => {
          return (<div>
            <Button type="link" onClick={async () => {
              this.props.history.push('/cloud/account/emailOrder/list?orderNo=' + record.orderNo)
            }}>查看</Button>
          </div>)
        }
      }
    ...
    return (<div>
      <Breadcrumb>
        ...
        <Breadcrumb.BreadcrumbItem>订单管理</Breadcrumb.BreadcrumbItem>
      </Breadcrumb>
