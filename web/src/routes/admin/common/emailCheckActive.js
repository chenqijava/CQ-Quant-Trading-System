...
class MyComponent extends Component {
  ...
  async load(pagination, filters, sorter) {
    ...
    let res = await axios.post(`/api/task/${pagination.pageSize}/${pagination.current}`, {
      filters: { ...filters, type: 'EmailCheckActive' }, sorter
    });
  ...
  cancelOrder() {
    ...
    DialogApi.warning({
      ...
      onOk: async () => {
        this.setState({ loading: true });
        let result = await axios.post(`/api/task/stop`, { ids });
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        ellipsis: true,
        render: (v, r) => {
          return r.params ? `${r.params.normalCount || 0}/${r.params.exceptCount || 0}/${r.params.unknownCount || 0}/${r.params.noExistsCount || 0}` : ''
        }
      },
      {
        title: '订单状态',
          ...
          return (
            <div style={{
              ...
              <Button
                ...
                onClick={async () => {
                  const response = await fetch('/api/consumer/res/download/' + r.params.filepath);
              ...
              {r.status === 'success' ? <Button
                ...
                onClick={() => {
                  this.setState({
                    orderVisible: true,
                    orders: [{name: r.params.normalFile},{name: r.params.exceptFile},{name: r.params.unknownFile},{name: r.params.noExistsFile}].filter(e => e.name)
                  })
                }}
              >订单报表
              </Button> : ''}
    ...
    return (<MyTranslate><ExternalAccountFilter>
      <Breadcrumb>
        ...
        <Breadcrumb.BreadcrumbItem>邮箱筛活</Breadcrumb.BreadcrumbItem>
      </Breadcrumb>
      ...
      <Dialog
        ...
        onConfirm={async () => {
          ...
          this.setState({ loading: true });
          // 过滤掉null
          let res = await axios.post(`/api/emailCheckActive/save`, form)
          if (res.data.code == 1) {
            message.success('操作成功')
