...
class MyComponent extends Component {
  ...
  async load(pagination, filters, sorter) {
    ...
    let res = await axios.post(`${baseUrl}/${pagination.pageSize}/${pagination.current}`, { filters: { ...filters, userID: this.props.userID }, sorter })
  ...
  async getBalanceChangeTotal (filters, sorter) {
    let res = await axios.post(`/api/consumer/user/balanceChange`, { filters: { ...filters, userID: this.props.userID }, sorter })
    if (res.data.code === 1) {
  ...
  render() {
    ...
    return (<div>
      <Breadcrumb>
        ...
        <BreadcrumbItem>我的账单</BreadcrumbItem>
      </Breadcrumb>
