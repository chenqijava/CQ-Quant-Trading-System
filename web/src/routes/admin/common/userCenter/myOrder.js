...
class MyComponent extends Component {
  ...
  async load(pagination, filters, sorter) {
    ...
    let res = await axios.post(`/api/buyEmailOrder/list/${pagination.pageSize}/${pagination.current}`, { filters: {...filters, userID: this.props.userID, type: {$in:[null,'v1']}}, sorter });
  ...
  render() {
        ...
        render: (t, record, index) => {
          return (<div>
            <Button type="link" onClick={async () => {
              this.props.history.push('/cloud/user/email?orderNo=' + record.orderNo)
            }}>查看</Button>
          </div>)
        }
      }
    ...
    return (<div>
      <Breadcrumb>
        ...
        <Breadcrumb.BreadcrumbItem>我的订单</Breadcrumb.BreadcrumbItem>
      </Breadcrumb>
