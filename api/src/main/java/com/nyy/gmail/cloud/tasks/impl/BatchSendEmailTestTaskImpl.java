...
@Slf4j
@Component("BatchSendEmailTest")
public class BatchSendEmailTestTaskImpl extends AbstractTask implements BaseTask {
    @Autowired
...
//                        String replacement = "href=\"" + baseUrl + "/api/latest/external/" + uuid + "/" + entry.getKey() + "\"";
...
//            content = content + "\n" + "<img src=\"" + baseUrl + "/api/latest/img/" + uuid + "/logo.png\" width=\"1\" height=\"1\" style=\"display:none;\" alt=\"\" />\n";
            content = BatchSendEmailTaskImpl.insertRandomImgSafe(content, "<img src=\"" + baseUrl + "/" + uuid + "/logo.png\" width=\"1\" height=\"1\" style=\"display:none;\" alt=\"\" />");
        }
...
//                "</html>";
    ...
    public boolean publishTask(GroupTask groupTask) {
        ...
        String content = ((Map)params.get("reqDto")).get("content").toString();
        String title = ((Map)params.get("reqDto")).get("title").toString();
        ...
        switch (GroupTaskStatusEnums.fromCode(groupTask.getStatus())) {
            ...
            case GroupTaskStatusEnums.init, GroupTaskStatusEnums.processing -> {
                ...
                if (!CollectionUtils.isEmpty(addDatas)) {
                    ...
                    int count = ((List<String>)params.get("addDatas")).size();
                    for (int i = 0; i < addDatas.size(); i ++) {
                        ...
                        // 创建任务
                        if (testAB.equals("yes") && taskCount >= count / 2) {
                            // B测
...
//                            }
                            SubTask subTask = taskUtil.newTaskModel(groupTask, Map.of(), new HashMap(Map.of(
                                    ...
                                    "email", split[0].trim(), "title", titleB, "isB", "1", "addData", addData,"sendEmailIntervalInSecond", params.getOrDefault("sendEmailIntervalInSecond","5"))), new Date());
...
//                            thisContent = processEmailContent(thisContent, subTask.getTmpId(), split[0].trim(), monitorOpen, monitorClick, addUnsubscribe);
        ...
        if (!subTaskList.isEmpty() && ids.isEmpty()) {
            // 可用账号不足，补充
