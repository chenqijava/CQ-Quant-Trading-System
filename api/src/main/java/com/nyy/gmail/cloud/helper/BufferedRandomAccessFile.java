...
public class BufferedRandomAccessFile extends RandomAccessFile {
    ...
    private static final int MIN_BUFSIZE = 128;
    ...
    private int fillBuffer() throws IOException {
        // log.info(String.format("Filling new buffer for %s", toString()));
    ...
    public final String getNextLine() throws IOException, EOFException {
        ...
        if (getFilePointer() == 0 && bufend > 3 && buffer[0] == (byte) 0xEF && buffer[1] == (byte) 0xBB && buffer[2] == (byte) 0xBF) {
          // 跳过 BOM（前三个字节）
          bufpos = 3;
        }
        ...
        if (lineend > 0 && buffer[lineend - 1] == '\r') {
            str = new String(buffer, bufpos, lineend - bufpos - 1);
        } else {
            str = new String(buffer, bufpos, lineend - bufpos);
        }
        bufpos = lineend + 1;
        return str;
    }
