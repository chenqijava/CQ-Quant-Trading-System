...
@Slf4j
@Component("BatchSendEmail")
public class BatchSendEmailTaskImpl extends AbstractTask implements BaseTask {
    @Autowired
...
//                "</html>";
    ...
    public boolean publishTask(GroupTask groupTask) {
        ...
        String useAiOptimize = ((Map)params.get("reqDto")).get("useAiOptimize").toString();
        String testAB = ((Map)params.get("reqDto")).get("testAB").toString();
        String testABStep = params.getOrDefault("testABStep", "1").toString();
        String content = testABStep.equals("1") ? ((Map)params.get("reqDto")).get("content").toString() : params.getOrDefault("content", "").toString();
        String title = testABStep.equals("1") ? ((Map)params.get("reqDto")).get("title").toString() : params.getOrDefault("title", "").toString();
        String contentB = ((Map)params.get("reqDto")).getOrDefault("contentB", "").toString();
        String titleB = ((Map)params.get("reqDto")).getOrDefault("titleB", "").toString();
        String monitorOpen =  ((Map)params.get("reqDto")).getOrDefault("monitorOpen", "").toString();
        String monitorClick =  ((Map)params.get("reqDto")).getOrDefault("monitorClick", "").toString();
        String addUnsubscribe =  ((Map)params.get("reqDto")).getOrDefault("addUnsubscribe", "").toString();
        String titleParams = ((Map)params.get("reqDto")).getOrDefault("titleParams", "").toString();
        String contentParams = ((Map)params.get("reqDto")).getOrDefault("contentParams", "").toString();
        String contactParams = ((Map)params.get("reqDto")).getOrDefault("contactParams", "").toString();
        String titleParamsFilepath = ((Map)params.get("reqDto")).getOrDefault("titleParamsFilepath", "").toString();
        String contentParamsFilepath = ((Map)params.get("reqDto")).getOrDefault("contentParamsFilepath", "").toString();
        String contactParamsFilepath = ((Map)params.get("reqDto")).getOrDefault("contactParamsFilepath", "").toString();
        List<String> systemEmails = ((List<String>)params.get("systemEmail"));
        Integer oneAccountSendLimit = Integer.parseInt(((Map)params.get("reqDto")).getOrDefault("oneAccountSendLimit", "20").toString());
        Integer failRatePauseTask = Integer.parseInt(((Map)params.get("reqDto")).getOrDefault("failRatePauseTask", "20").toString());
        Integer availableAccountPauseTask = Integer.parseInt(((Map)params.get("reqDto")).getOrDefault("availableAccountPauseTask", "20").toString());
        Integer spamRatePauseTask = Integer.parseInt(((Map)params.get("reqDto")).getOrDefault("spamRatePauseTask", "20").toString());
        String closeFailRatePauseTask = ((Map)params.get("reqDto")).getOrDefault("closeFailRatePauseTask", "disable").toString();
        String closeAvailableAccountPauseTask = ((Map)params.get("reqDto")).getOrDefault("closeAvailableAccountPauseTask", "disable").toString();
        String closeSpamRatePauseTask = ((Map)params.get("reqDto")).getOrDefault("closeSpamRatePauseTask", "disable").toString();
        ...
        if (content.getBytes().length > 10 * 1024 * 1024) {
            // 内容太大
            content = "邮件内容过大被TNT屏蔽";
        }

        if (content.getBytes().length > 10 * 1024 * 1024) {
            // 内容太大
            contentB = "邮件内容过大被TNT屏蔽";
        }

        List<String> sendedSystemEmail = (List<String>)params.getOrDefault("sendedSystemEmail", new ArrayList<>());
        ...
        switch (GroupTaskStatusEnums.fromCode(groupTask.getStatus())) {
            ...
            case GroupTaskStatusEnums.init, GroupTaskStatusEnums.processing -> {
                ...
                // 计算步长
                int step = -1;
                if (systemEmails != null && !systemEmails.isEmpty()) {
                    step = Math.round((float) groupTask.getTotal() / (systemEmails.size() + 1));
                }

                long taskCount = subTaskRepository.countByGroupTaskIdEquals(groupTask.get_id());
                List<String> addDatas = null;
                if (testAB.equals("yes") && testABStep.equals("1")) {
                    // AB测数量
                    String percent = ((Map)params.get("reqDto")).get("percent").toString();
                    long count = groupTask.getPublishTotalCount() * Long.parseLong(percent) / 100;
                    if (count % 2 != 0) {
                        count = count - 1;
                    }
                    if (count <= taskCount) {
                        ...
                        addDatas = taskUtil.getAddData(groupTask.getParams(), (int) (count - taskCount));

                        // 插入种子邮箱
                        List<String> tmp = new ArrayList<>();
                        if (addDatas != null) {
                            for (int i = 0; i < addDatas.size(); i++) {
                                int idx = Math.toIntExact(taskCount + i);
                                int current = -1;
                                if (step > 0) {
                                    current = idx / step - 1;
                                }
                                if (step > 0 && idx > 0 && (idx % step) == 0) {
                                    if (current < systemEmails.size()) {
                                        if (!sendedSystemEmail.contains(systemEmails.get(current))) {
                                            sendedSystemEmail.add(systemEmails.get(current));
                                            tmp.add(systemEmails.get(current));
                                        }
                                    }
                                }
                                tmp.add(addDatas.get(i));
                            }
                        }
                        if (CollectionUtils.isEmpty(addDatas) && step > 0) {
                            int current = Math.toIntExact(taskCount / (step + 1));
                            for (int i = current; i < systemEmails.size(); i++) {
                                if (!sendedSystemEmail.contains(systemEmails.get(current))) {
                                    sendedSystemEmail.add(systemEmails.get(current));
                                    tmp.add(systemEmails.get(current));
                                }
                    ...
                    addDatas = taskUtil.getAddData(groupTask.getParams(), maxInsertCount);
                ...
                if (!CollectionUtils.isEmpty(addDatas)) {
                    ...
                    if (StringUtil.isNotEmpty(titleParamsFilepath)) {
                        Pair<List<String>, Long> filepath = FileUtils.readFileLines(Path.of(pathConfig.getResBak(), titleParamsFilepath), 0, 100000);
                        titles = filepath.getLeft();
                    } else if (StringUtil.isNotEmpty(titleParams)) {
                        titles = Arrays.stream(titleParams.split("\n")).filter(StringUtils::isNotBlank).toList();
                    }
                    if (StringUtil.isNotEmpty(contentParamsFilepath)) {
                        Pair<List<String>, Long> filepath = FileUtils.readFileLines(Path.of(pathConfig.getResBak(), contentParamsFilepath), 0, 100000);
                        contents = Arrays.stream(String.join("<br/>\n", filepath.getLeft()).split("==NEWLINE==")).filter(StringUtils::isNotBlank).toList();
                    } else if (StringUtil.isNotEmpty(contentParams)) {
                        contents = Arrays.stream(contentParams.split("==NEWLINE==")).filter(StringUtils::isNotBlank).toList();
                    }
                    if (StringUtil.isNotEmpty(contactParamsFilepath)) {
                        Pair<List<String>, Long> filepath = FileUtils.readFileLines(Path.of(pathConfig.getResBak(), contactParamsFilepath), 0, 100000);
                        contacts = filepath.getLeft();
                    } else if (StringUtil.isNotEmpty(contactParams)) {
                        contacts = Arrays.stream(contactParams.split("\n")).filter(StringUtils::isNotBlank).toList();
                    }
                    ...
                    for (int i = 0; i < addDatas.size(); i ++) {
                        ...
                        String titleParam = "";
                        if (!titles.isEmpty()) {
                            titleParam = titles.get((int)taskCount % titles.size());
                        }
                        String contentParam = "";
                        if (!contents.isEmpty()) {
                            contentParam = contents.get((int)taskCount % contents.size());
                        }
                        String contactParam = "";
                        if (!contacts.isEmpty()) {
                            contactParam = contacts.get((int)taskCount % contacts.size());
                        }

                        // 创建任务
                        if (testAB.equals("yes") && testABStep.equals("1") && taskCount >= count / 2) {
                            // B测
...
//                            }
                            SubTask subTask = taskUtil.newTaskModel(groupTask, Map.of(), new HashMap(Map.of(
                                    "titleParam", titleParam, "contentParam", contentParam, "contactParam", contactParam,
                                    "email", split[0].trim(), "title", titleB, "isB", "1", "addData", addData,
                                    "testABStep", testABStep,"sendEmailIntervalInSecond", params.getOrDefault("sendEmailIntervalInSecond","5"))), new Date());
//                            subTask.setTmpId(UUIDUtils.get32UUId());
//                            // 替换
                            String thisContent = contentB;
                            thisContent = processingBase64Img(thisContent);
                            if (!contentB.equals(thisContent)) {
                                contentB = thisContent;
                                ((Map)params.get("reqDto")).put("contentB", contentB);
                            }
                            Map<String, String> map = new HashMap<>();
                            map.put("NEWLINE", "<br/>\n");
                            if (StringUtils.isNotBlank(titleParam)) {
                                map.put("title", TemplateUtils.replaceTemplateVars(titleParam, map));
                            }
                            if (StringUtils.isNotBlank(contentParam)) {
                                contentParam = contentParam.replace("\n", "<br/>\n");
                                map.put("content", TemplateUtils.replaceTemplateVars(contentParam, map));
                            }
                            if (StringUtils.isNotBlank(contactParam)) {
                                map.put("contact", TemplateUtils.replaceTemplateVars(contactParam, map));
                            }
...
//                                // 标题优化
//                                subTask.getParams().put("title", aiOptimizeTitle(titleB, groupTask.getUserID()));
//                            }
//                            thisContent = TemplateUtils.replaceTemplateVars(thisContent, map);
//                            thisContent = processEmailContent(thisContent, subTask.getTmpId(), split[0].trim(), monitorOpen, monitorClick, addUnsubscribe);
...
//                            }
                            SubTask subTask = taskUtil.newTaskModel(groupTask, Map.of(), new HashMap(Map.of(
                                    "titleParam", titleParam, "contentParam", contentParam, "contactParam", contactParam,
                                    "email", split[0].trim(), "title", title, "addData", addData,
                                    "testABStep", testABStep, "sendEmailIntervalInSecond", params.getOrDefault("sendEmailIntervalInSecond","5"))), new Date());
                            subTask.setTmpId(UUIDUtils.get32UUId());
                            // 替换
                            ...
                            if (!content.equals(thisContent)) {
                                content = thisContent;
                                ((Map)params.get("reqDto")).put("content", content);
                            }
                            Map<String, String> map = new HashMap<>();
                            map.put("NEWLINE", "<br/>\n");
                            if (StringUtils.isNotBlank(titleParam)) {
...
//                                thisContent = aiOptimizeV2(thisContent, groupTask.getUserID());
//                                // 标题优化
//                                subTask.getParams().put("title", aiOptimizeTitle(title, groupTask.getUserID()));
//                            }
//                            thisContent = TemplateUtils.replaceTemplateVars(thisContent, map);
//                            thisContent = processEmailContent(thisContent, subTask.getTmpId(), split[0].trim(), monitorOpen, monitorClick, addUnsubscribe);
                            subTask.getParams().put("content", thisContent);
                    ...
                    groupTask.getParams().put("sendedSystemEmail", sendedSystemEmail);
                    ...
                    if (testAB.equals("yes") && testABStep.equals("1")) {
                            groupTask.setTotal(groupTask.getPublishTotalCount());
        ...
        // 判断暂停任务
        if (closeFailRatePauseTask.equals("disable")) {
            ...
            if (success + failed > 100) {
                if (failed >= ((double)(success + failed)) * failRatePauseTask / 100) {
        ...
        // 判断种子邮箱垃圾率
        if (closeSpamRatePauseTask.equals("disable")) {
            List<Message> messageList = null;
            if (systemEmails != null && !systemEmails.isEmpty()) {
                messageList = messageRepository.findByEmailsAndCreateTime(systemEmails, groupTask.getCreateTime());
                log.info("groupTask {} 检查种子邮箱的邮件，messageList size {}", groupTask.get_id(), messageList.size());
            }
            if (messageList != null) {
                ...
                log.info("groupTask {} 是属于账号的 accountList Size {}", groupTask.get_id(), accountList.size());
                int smapNum = 0;
                int totalNum = 0;
                for (Message message : messageList) {
                    List<Account> list = accountList.stream().filter(e -> e.getEmail().equals(message.getSender()) && !e.get_id().equals(message.getAccId())).toList();
                    if (!list.isEmpty()) {
                        subTaskRepository.updateResultLabel(String.join(",", message.getLabels()), list.getFirst().get_id(), message.getEmail(), groupTask.get_id());
                        totalNum++;
                        if (message.getLabels() != null && message.getLabels().contains("^s")) {
                            smapNum++;
                        }
                    }
                }
                log.info("groupTask {} 垃圾邮件 {}/{}", groupTask.get_id(), smapNum, totalNum);
                if (totalNum > 10 && ((double)smapNum / totalNum * 100) >= spamRatePauseTask) {
                    groupTask.setStatus(GroupTaskStatusEnums.pause.getCode());
                    groupTaskRepository.save(groupTask);
                    return true;
                }
        ...
        if (!subTaskList.isEmpty() && ids.size() < 50 && subTaskList.size() > ids.size()) {
            ...
            // 检查账号
            if (closeAvailableAccountPauseTask.equals("disable")) {
                if (availableAccountPauseTask > ids.size() && subTaskList.size() > availableAccountPauseTask) {
                    groupTask.setStatus(GroupTaskStatusEnums.pause.getCode());
                    groupTaskRepository.save(groupTask);
                    return true;
                }
        ...
        List<SubTask> restart = subTaskRepository.findRestart(groupTask.get_id(), calendar2.getTime());
            ...
            if (subTaskList != null) {
                if (subTaskList.isEmpty() && groupTask.getSuccess() + groupTask.getFailed() >= groupTask.getTotal()) {
                    ...
                    if (subTaskList.isEmpty()) {
                        ...
                        // A/B测第二阶段
                        if (testAB.equals("yes") && testABStep.equals("1") && groupTask.getSuccess() + groupTask.getFailed() >= groupTask.getPublishedCount()) {
                            String factor = ((Map)params.get("reqDto")).getOrDefault("factor", "").toString();
                            if (factor.equals("other")) {
                                groupTask.setPublishStatus("success");
                            } else {
                                String testTimeLengthHour = ((Map)params.get("reqDto")).getOrDefault("testTimeLengthHour", "1").toString();
                                Calendar calendar = Calendar.getInstance();
                                calendar.setTime(groupTask.getExecuteTime());
                                calendar.add(Calendar.MINUTE, (int) (Double.parseDouble(testTimeLengthHour) * 60));
                                if (new Date().after(calendar.getTime())) {
                                    Map<String, String> map = selectContentAndTitle(groupTask);
                                    groupTask.getParams().put("testABStep", "2");
                                    groupTask.getParams().put("content", map.get("content"));
                                    groupTask.getParams().put("title", map.get("title"));
                                }
                            }
                        }
                    }
...
//                String replacement = "src=\"" + baseUrl + "/api/latest/img/uuid/" + img1 + "\"";
    ...
    public boolean runTask(SubTask task, Account account) {
        ...
        try {
            ...
            String titleParam = task.getParams().getOrDefault("titleParam", "").toString();
            String contentParam = task.getParams().getOrDefault("contentParam", "").toString();
            String contactParam = task.getParams().getOrDefault("contactParam", "").toString();
            ...
            try {
                ...
                thisContent = TemplateUtils.replaceTemplateVars(thisContent, map);
                content = processEmailContent(thisContent, wt.getTmpId(), split[0].trim(), monitorOpen, monitorClick, addUnsubscribe, wt.get_id());
            } catch (Exception e) {
            ...
            if (sendEmailResponse != null && sendEmailResponse.getCode().equals(0)) {
                ...
                if ((account.getType().equals(AccountTypeEnums.sendgrid.getCode())
                                ...
                                && StringUtils.isNotEmpty(sendEmailResponse.getMsg())) {
