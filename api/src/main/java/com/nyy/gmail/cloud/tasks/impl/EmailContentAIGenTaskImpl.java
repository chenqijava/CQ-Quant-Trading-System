...
@Slf4j
@Component("EmailContentAiGen")
public class EmailContentAIGenTaskImpl extends AbstractTask implements BaseTask {
    @Autowired
...
//                "</html>";
    ...
    public boolean publishTask(GroupTask groupTask) {
        ...
        String subject = ((Map)params.get("reqDto")).getOrDefault("subject", "").toString();
        String content = ((Map)params.get("reqDto")).getOrDefault("content", "").toString();
        String genNum = ((Map)params.get("reqDto")).getOrDefault("genNum", "10").toString();
        String aiModel = ((Map)params.get("reqDto")).getOrDefault("aiModel", "").toString();
        List<String> emotion = (List<String>) ((Map)params.get("reqDto")).getOrDefault("emotion", "");
        String target = ((Map)params.get("reqDto")).getOrDefault("target", "").toString();
        String character = ((Map)params.get("reqDto")).getOrDefault("character", "").toString();
        String style = ((Map)params.get("reqDto")).getOrDefault("style", "").toString();
        String other = ((Map)params.get("reqDto")).getOrDefault("other", "").toString();
        String source = ((Map)params.get("reqDto")).getOrDefault("source", "").toString();
        String message = ((Map)params.get("reqDto")).getOrDefault("contentInfo", "").toString();
...
//                int maxInsertCount = 5000;
                ...
                if (taskCount <= 0) {
                    ...
                    content = processingBase64Img(content);
                    String prompt = null;
                    if (source.equals("3")) {
                        prompt = content + "\n" +
                                "要求： 只返回严格格式的 JSON 数组, 数组中的对象包括三个字段：lang(邮件正文的主体语言)、title(修改后的邮件标题)、email(修改后的邮件正文)、score(对本次修改进行打分数)，不得包含任何额外文字或解释。";
                        List<List<String>> generateContents = aiGenerateContent("", "", prompt, new HashMap<>(), aiModel, 1);
                        if (CollectionUtils.isNotEmpty(generateContents)
                                && CollectionUtils.isNotEmpty(generateContents.getFirst())
                                && generateContents.getFirst().size() == 2
                        ) {
                            content = generateContents.getFirst().get(1);
                        }
                    }
//                    List<String> subjects = aiGenerateTitle(subject, null,
//                            Map.of("emotion", StringUtils.join(emotion, ","), "target", target, "character", character, "style", style, "other", other, "message", message),
//                            aiModel, Integer.valueOf(genNum));
                    List<List<String>> generateContents = aiGenerateContent(content, subject, null,
                            Map.of("emotion", StringUtils.join(emotion, ","), "target", target, "character", character, "style", style, "other", other, "message", message),
                            aiModel, Integer.valueOf(genNum));
                    for (int i = 0; i < generateContents.size(); i ++) {
                        List<String> contents = generateContents.get(i);
                        if (CollectionUtils.isEmpty(contents) || contents.size() != 2) {
                            continue;
                        }
                        if (StringUtils.isEmpty(contents.get(0))) {
                            continue;
                        }
                        if (StringUtils.isEmpty(contents.get(1))) {
                            continue;
                        }
                        String newSubject = contents.get(0);
                        String addData = contents.get(1);
                        for (String email : systemEmails) {
                            SubTask subTask = taskUtil.newTaskModel(groupTask, Map.of(), new HashMap(Map.of(
                                    "email", email, "title", newSubject, "content", addData,
                                    "emotion", StringUtils.join(emotion, ","), "target", target, "character", character,
                                    "style", style, "other", other, "message", message,
...
//                String replacement = "src=\"" + baseUrl + "/api/latest/img/uuid/" + img1 + "\"";
    ...
    private List<List<String>> aiGenerateContent(String content, String subject, String prompt, Map<String, String> param, String model, int genNum) {
        AiTypeEnums aiTypeEnums = AiTypeEnums.GoogleStudio;
        if (model.equals(AiTypeEnums.Chatgpt.getCode())) {
            aiTypeEnums = AiTypeEnums.Chatgpt;
        }
        String apiKey = "AIzaSyCCHis8Wd2uEqMeGXTMMk4-F8aCzZzJZHE";
        int n = 10;
        if (content.length() > 820) {
            n = 5;
        }
        List<List<String>> list = new ArrayList<>();
        for (int i = 0; i < 100; i++) {
            try {
                AiImageRecognition imageRecognition = applicationContext.getBean(aiTypeEnums.getCode(), AiImageRecognition.class);
                String contentOptimize = imageRecognition.emailContentGenerateV2(content, subject, param, apiKey, prompt, n - i);
                if (StringUtils.isNotEmpty(contentOptimize)) {
                    contentOptimize = contentOptimize.trim();
                    if (contentOptimize.startsWith("```json")) {
                        contentOptimize =  contentOptimize.substring("```json".length());
                    ...
                    for (int j = 0; j < jsonArray.size(); j++) {
                        list.add(List.of(jsonArray.getJSONObject(j).getString("title"), jsonArray.getJSONObject(j).getString("email")));
                        if (list.size() >= genNum) {
                            return list;
                        }
                    }
